...
{
  /* per-address buffer: X/Y fired back-to-back every frame share one static
     cmd[] -> second write overwrites first while DMA still reads it -> X gets
     Y bytes (or partial), looks reversed/lost. Separate buffers fix it. */
  __IO static uint8_t cmd1[16] = {0};   /* addr 1 */
  __IO static uint8_t cmd2[16] = {0};   /* addr 2 */
  __IO uint8_t *cmd = (addr == 2) ? cmd2 : cmd1;
  ...
  /* Wait previous DMA TX fully completes (gState READY) before next frame,
     so this buffer is not read & overwritten mid-flight. */
  {
    uint32_t _to = 5000;
    while ((huart1.gState != HAL_UART_STATE_READY) && --_to) { ; }
  }
